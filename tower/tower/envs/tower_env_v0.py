...
class TowerEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Discrete(TARGET_TOWER_HEIGHT+1)
    ...
    def _get_game_state(self):
        return [adjacent_tower_size(agent.x, agent.direction, self.block_grid) for agent in self.agents]
    ...
    # Render the environment to the screen
        if mode == 'console':
            ...
            for agent in self.agents:
                ...
                printfont.render_to(self.screen, (rx+CELL_SIZE//2, ry+CELL_SIZE//2), str(agent.holding_blocks), (0, 10, 10))
                pygame.display.flip()
    ...
    def step(self, actions):
        ...
        for i, agent in enumerate(self.agents):
            ...
            reward += self._get_reward(action, tower_height_before, tower_heights_after[i])
        ...
        return tower_heights_after, reward, done, result

    def _get_reward(self, action, t1, t2):
        if action == 0: return -1/self.num_agents
        if t1 == 0 and action == 1: return -20/self.num_agents
        if t2 == TARGET_TOWER_HEIGHT: return 50/self.num_agents
        if t2 > t1 and action == 1: return 5/self.num_agents
        
        else: return 0/self.num_agents
